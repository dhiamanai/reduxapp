import React from 'react';

import { Navbar } from 'flowbite-react';

export default function Navbaar() {
    return (
        <Navbar fluid rounded className='bg-violet-100 shadow-xl border-solid border-2 border-b-red-500 '>
            <Navbar.Brand  href="https://flowbite-react.com">
                <svg className="w-[45px] h-[45px] text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    <path fillRule="evenodd" d="M22 5.892a8.178 8.178 0 0 1-2.355.635 4.074 4.074 0 0 0 1.8-2.235 8.343 8.343 0 0 1-2.605.981A4.13 4.13 0 0 0 15.85 4a4.068 4.068 0 0 0-4.1 4.038c0 .31.035.618.105.919A11.705 11.705 0 0 1 3.4 4.734a4.006 4.006 0 0 0 1.268 5.392 4.165 4.165 0 0 1-1.859-.5v.05A4.057 4.057 0 0 0 6.1 13.635a4.192 4.192 0 0 1-1.856.07 4.108 4.108 0 0 0 3.831 2.807A8.36 8.36 0 0 1 2 18.184 11.732 11.732 0 0 0 8.291 20 11.502 11.502 0 0 0 19.964 8.5c0-.177 0-.349-.012-.523A8.143 8.143 0 0 0 22 5.892Z" clipRule="evenodd"/>
                </svg>
            </Navbar.Brand>

           
            <Navbar.Toggle />
            <Navbar.Collapse>
                <Navbar.Link className='hover:bg-gray-200 active:bg-violet-700' href="#" active>
                         Home
                </Navbar.Link>
                <Navbar.Link className='hover:bg-gray-200 active:bg-violet-700'  href="#" active>
                         About
                </Navbar.Link>
                <Navbar.Link className='hover:bg-gray-200 active:bg-violet-700' href="#" active>Services</Navbar.Link>
                <Navbar.Link className='hover:bg-gray-200 active:bg-violet-700' href="#" active>Pricing</Navbar.Link>
                <Navbar.Link className='hover:bg-gray-200 active:bg-violet-700' href="#" active>Contact</Navbar.Link>
            </Navbar.Collapse>
        </Navbar>
    );
}